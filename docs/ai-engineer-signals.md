...
| **Multi-agent orchestration** | Jarvis / Leo / Alfred on **deliberately different models** ([README "The staff"](../README.md#the-staff)) | Role *and* model diversity so agents don't share blind spots: a Claude-based builder, a self-hosted Hermes contrarian (read-only canon), a scoped homelab sysadmin. One debates, the human decides. Cockpit: `thousand-sunny`. |
